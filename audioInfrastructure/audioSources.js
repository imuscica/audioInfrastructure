class OscillatorClass{
	...
	setAmplitude(amp){
		this.maxVolume = amp;
		if (this.isPlaying) {
			this.gainNode.gain.linearRampToValueAtTime(this.maxVolume,this.context.currentTime + 0.05);
		}
	}
	...
	startNote(freq){
		this.freq = freq;
		if (!this.isPlaying){
			this.gainNode.gain.linearRampToValueAtTime(this.maxVolume,this.context.currentTime + 0.1);
			this.isPlaying = true;
		}
	}
	stopNote(freq){
	...
	playNote(freq, startTime, duration){
		...
		this.gainNode.gain.linearRampToValueAtTime(0,ct);
		this.gainNode.gain.linearRampToValueAtTime(this.maxVolume,ct+attack);
		this.gainNode.gain.linearRampToValueAtTime(sustainVal,ct+attack+decay);
		this.gainNode.gain.linearRampToValueAtTime(sustainVal,ct+attack+decay+sustain);
		this.gainNode.gain.linearRampToValueAtTime(0,ct+attack+decay+sustain+release);
